...
import { generateDetailedLessonContent, generateQuizForLesson } from '../services/geminiService';
...
export const LessonEditor: React.FC<LessonEditorProps> = ({ lesson, courseTitle, onSave, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-slate-900/40 backdrop-blur-md animate-in fade-in duration-500 font-sans">
      <div className="w-full max-w-6xl h-[94vh] bg-white flex flex-col shadow-2xl rounded-[2rem] overflow-hidden">
        ...
        <div className="px-12 py-10 flex items-center justify-between bg-white border-b border-slate-100">
          <div className="flex items-center gap-5">
             <div className="w-[4px] h-10 bg-[#4f46e5] rounded-full"></div>
             <h2 className="text-2xl font-black uppercase tracking-[0.4em] text-[#4f46e5]">Studio Haute Couture</h2>
          </div>
        ...
        <div className="flex-grow overflow-y-auto px-16 py-12 space-y-16 no-scrollbar pb-40 bg-slate-50/5">
          <div className="space-y-4">
            ...
            <input 
              ...
              className="editorial-title text-6xl bg-transparent border-none focus:outline-none w-full text-slate-900 tracking-tighter"
              placeholder="Unité de savoir..."
            />
          </div>
          ...
          <div className="space-y-12">
            {editedLesson.blocks.map((block) => (
              <div 
                key={block.id} 
                className={`relative bg-white border ${block.type === ContentBlockType.QUIZ ? 'border-[#4f46e5] ring-1 ring-[#4f46e5]/10 shadow-xl shadow-[#4f46e5]/5' : 'border-slate-100'} rounded-[3rem] p-12 group transition-all`}
              >
                <button 
                    ...
                    <div className="grid gap-4">
                      {block.payload?.questions?.map((q: any, qi: number) => (
                        <div key={qi} className="p-6 bg-slate-50/50 rounded-2xl border border-slate-100 flex items-center justify-between">
                           ...
                           <span className="text-[9px] font-black uppercase text-emerald-500 bg-emerald-50 px-3 py-1 rounded-full">IA Validé</span>
                        </div>
                      ))}
                    </div>
                    <button 
                      onClick={() => {
                        const newQ: QuizQuestion = { question: "Nouvelle question ?", options: ["Choix 1", "Choix 2"], correctIndex: 0 };
                        updateBlock(block.id, { payload: { ...block.payload, questions: [...(block.payload?.questions || []), newQ] } });
                      }}
                      className="w-full py-8 border-2 border-dashed border-slate-100 rounded-[2.5rem] text-[9px] font-black uppercase tracking-[0.3em] text-slate-300 hover:text-[#4f46e5] hover:border-[#4f46e5]/20 transition-all flex items-center justify-center gap-4"
                    >
        ...
        {/* Floating Toolbar as per screenshot */}
        <div className="absolute bottom-12 left-1/2 -translate-x-1/2 flex items-center bg-white rounded-full shadow-2xl border border-slate-100 p-2.5 z-50 animate-in slide-in-from-bottom-8">
           ...
           <button 
             ...
             className="px-12 py-5 bg-[#4f46e5] text-white rounded-full text-[10px] font-black uppercase tracking-[0.4em] shadow-xl flex items-center gap-4 hover:scale-105 transition-all"
           >
